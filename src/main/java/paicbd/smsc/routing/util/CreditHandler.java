...
public class CreditHandler {
	...
	private final LoadServiceProviders serviceProviderLoader;
	...
	public Flux<Map<String, Object>> getCreditUsedByServiceProvider(ConcurrentMap<Integer, AtomicInteger> counter) {
		...
		return Flux.fromIterable(counter.entrySet())
				...
				.mapNotNull(entry -> {
					String systemId = serviceProviderLoader.getSystemIdByNetworkId(entry.getKey());
					if (systemId != null) {
						return Map.of("systemId", systemId, "creditUsed", entry.getValue().get());
					}
					return null;
				});
	}
	...
	public void removeCreditUsed(String systemId) {
		creditUsed.remove(serviceProviderLoader.getNetworkIdBySystemId(systemId));
	}
}
